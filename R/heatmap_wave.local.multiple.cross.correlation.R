  ...
  function(Lst, lmax, lag.first=FALSE, xaxt="s", ci=NULL, pdf.write=NULL) {
    ...
    if(lag.first){
      ...
      for(i in c(-lmax:0,lmax:1)+lmax+1) {
        ...
        if (is.null(ci)||ci=="center") {val <- sapply(cor,function(x){x[["vals"]]})
        } else if (ci=="lower") {val <- sapply(cor,function(x){x[["lower"]]})
        } else if (ci=="upper"){val <- sapply(cor,function(x){x[["upper"]]})}
        colkey <- FALSE
